		...
		loop1: while(true) {
			...
			case 1: // 학생성적 추가
				...
				for(int i=0; i<name.length; i++) {
					System.out.println("이름 입력(0. 상위메뉴 이동)>>");
					name[i] = scan.next();
					//비교
					if(name[i].equals("0")) {
			...
			case 2://학생성적 출력
				// 상단 타이틀 출력
				for(int i=0; i<title.length; i++) {//이름 국어 영어~ 등으로 찍힘
				...
				break;
			case 3://학생성적 수정
				// 수정할 학생의 이름을 입력 → 어떤 과목 점수를 수정할지 정함 → 수정
...
//			            for(int i=search_num1[0]; i<(search_num1[0]+1); i++) {
			...
			default:
				System.out.println("번호가 잘 못 입력되었습니다. 다시 입력하세요!");
				break;
			}
		}
